...
const Topbar = ({ onSearch, onToggleTweaks, onToggleTheme, theme, crumbs, onCreateArticle, currentUser, onLogout, onManageUsers }) => {
  ...
  const roleLabel = { admin: 'Admin', collaborator: 'Colaborador', viewer: 'Visualizador' }[u.role] || u.role;
  const roleColor = { admin: 'oklch(0.5 0.18 30)', collaborator: 'oklch(0.5 0.18 255)', viewer: 'oklch(0.5 0.12 145)' }[u.role];
  ...
  return (
    <div className="topbar">
      ...
      <div className="top-actions">
        ...
        {onCreateArticle && (
          <button className="btn-primary" onClick={onCreateArticle}>
          ...
          {userMenuOpen && (
            <div onClick={e => e.stopPropagation()} style={{
              ...
              <div style={{ padding: '14px 16px', borderBottom: '1px solid var(--border)' }}>
                <div style={{ fontWeight: 600, fontSize: 14, color: 'var(--text)', marginBottom: 2 }}>{u.name}</div>
              ...
              <div style={{ padding: '6px 0' }}>
                ...
                <button onClick={() => { setUserMenuOpen(false); onLogout && onLogout(); }}
                  style={{ width: '100%', padding: '9px 16px', textAlign: 'left', background: 'none', border: 'none', cursor: 'pointer', fontSize: 13.5, color: 'oklch(0.5 0.18 30)', display: 'flex', alignItems: 'center', gap: 10, transition: 'background .1s' }}
                  onMouseEnter={e => e.currentTarget.style.background = 'oklch(0.97 0.02 30)'}
                  onMouseLeave={e => e.currentTarget.style.background = 'none'}
                >
                  <Icon name="arrow" size={15} style={{ transform: 'rotate(180deg)', color: 'oklch(0.6 0.15 30)' }} />
                  Cerrar sesión
                </button>
              </div>
            </div>
...
const Sidebar = ({
  ...
  return (
    <aside className="sidebar">
      ...
      <div style={{ marginTop: 'auto', padding: '12px 10px 8px', borderTop: '1px solid var(--border)' }}>
        <button
          className="nav-item"
          style={{ width: '100%', gap: 8, display: 'flex', alignItems: 'center' }}
          onClick={onManage}
        >
          <Icon name="sliders" size={15} className="ico" />
          <span className="label">Administrar estructura</span>
        </button>
      </div>
    </aside>
  );
